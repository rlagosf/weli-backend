// src/routers/promocion_tipos_pago.ts
...
async function getPromocionTipoPago(academiaId: number, id: number) {
  const [rows]: any = await db.query(
    ...
    FROM promocion_tipo_pago ptp

    INNER JOIN promociones_academia p
      ON p.id = ptp.promocion_id
     AND p.academia_id = ptp.academia_id
...
async function existsRelation(academiaId: number, promocionId: number, tipoPagoId: number, excludeId?: number) {
  const values: any[] = [academiaId, promocionId, tipoPagoId];
...
/**
 * Si existe un cargo con la misma academia,
 * promoción y tipo de pago, entonces esta relación
 * ya fue utilizada históricamente.
 */
async function relationHasCargos(academiaId: number, promocionId: number, tipoPagoId: number) {
  const [rows]: any = await db.query(
    `
    SELECT id
    FROM cargos_jugador
    ...
    LIMIT 1
    `,
    [academiaId, promocionId, tipoPagoId]
  );
...
export default async function promocion_tipos_pago(app: FastifyInstance) {
  ...
  app.get("/health", { preHandler: canRead }, async (req: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      return reply.code(500).send({
        ok: false,
        message: "Error en módulo promocion_tipos_pago",
      });
    }
  });
  ...
  app.get("/", { preHandler: canRead }, async (req: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      return reply.code(500).send({
        ok: false,
        message: "Error al listar relaciones promoción-tipo de pago",
      });
    }
  });
  ...
  app.get("/:id", { preHandler: canRead }, async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      const academiaId = resolveAcademiaId(req);
      const row = await getPromocionTipoPago(academiaId, parsed.data.id);
      ...
      if (!row) {
        return reply.code(404).send({
          ok: false,
          message: "Relación promoción-tipo de pago no encontrada",
        });
      }
      ...
      return reply.code(500).send({
        ok: false,
        message: "Error al obtener relación promoción-tipo de pago",
      });
    }
  });
  ...
  app.post("/", { preHandler: canWrite }, async (req: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const duplicate = await existsRelation(academiaId, body.promocion_id, body.tipo_pago_id);
      ...
      if (duplicate) {
        ...
        return reply.code(409).send({
          ok: false,
          message: "La promoción ya está asociada a este tipo de pago",
        });
      }

      const [result]: any = await db.query(
        `
        INSERT INTO promocion_tipo_pago
          (academia_id, promocion_id, tipo_pago_id)
        VALUES (?, ?, ?)
        `,
        [academiaId, body.promocion_id, body.tipo_pago_id]
      );

      const insertId = Number(result?.insertId);
      const row = await getPromocionTipoPago(academiaId, insertId);
      ...
      if (err?.errno === 1452 || err?.code === "ER_NO_REFERENCED_ROW_2") {
        return reply.code(409).send({
          ok: false,
          message: "La promoción o tipo de pago indicado no existe",
        });
      }
      ...
      return reply.code(500).send({
        ok: false,
        message: "Error al asociar promoción con tipo de pago",
      });
    }
  });
  ...
  app.put("/:id", { preHandler: canWrite }, async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const current = await getPromocionTipoPago(academiaId, id);
      ...
      const changed =
        body.promocion_id !== Number(current.promocion_id) || body.tipo_pago_id !== Number(current.tipo_pago_id);

      if (
        changed &&
        (await relationHasCargos(academiaId, Number(current.promocion_id), Number(current.tipo_pago_id)))
      ) {
        reply.header("Cache-Control", "no-store");

        return reply.code(409).send({
          ok: false,
          message: "La relación promoción-tipo de pago ya fue utilizada en cargos y no puede modificarse",
        });
      }
      ...
      const duplicate = await existsRelation(academiaId, body.promocion_id, body.tipo_pago_id, id);
      ...
      if (duplicate) {
        ...
        return reply.code(409).send({
          ok: false,
          message: "Ya existe otra asociación entre esta promoción y este tipo de pago",
        });
      }

      const [result]: any = await db.query(
        `
        UPDATE promocion_tipo_pago
        SET
          promocion_id = ?,
          tipo_pago_id = ?
        WHERE id = ?
        ...
        LIMIT 1
        `,
        [body.promocion_id, body.tipo_pago_id, id, academiaId]
      );
      ...
      const updated = await getPromocionTipoPago(academiaId, id);
      ...
      return reply.code(500).send({
        ok: false,
        message: "Error al actualizar relación promoción-tipo de pago",
      });
    }
  });
  ...
  app.patch("/:id", { preHandler: canWrite }, async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const changed =
        merged.promocion_id !== Number(current.promocion_id) || merged.tipo_pago_id !== Number(current.tipo_pago_id);
      ...
      const duplicate = await existsRelation(academiaId, merged.promocion_id, merged.tipo_pago_id, id);
      ...
      const [result]: any = await db.query(
        ...
        LIMIT 1
        `,
        [merged.promocion_id, merged.tipo_pago_id, id, academiaId]
      );
  ...
  app.delete("/:id", { preHandler: canWrite }, async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      if (await relationHasCargos(academiaId, Number(current.promocion_id), Number(current.tipo_pago_id))) {
        reply.header("Cache-Control", "no-store");

        return reply.code(409).send({
          ok: false,
          message: "La relación promoción-tipo de pago ya fue utilizada para generar cargos y no puede eliminarse",
        });
      }

      const [result]: any = await db.query(
        `
        DELETE FROM promocion_tipo_pago
        WHERE id = ?
          AND academia_id = ?
        LIMIT 1
        `,
      ...
      return reply.code(500).send({
        ok: false,
        message: "Error al eliminar relación promoción-tipo de pago",
      });
    }
  });
}
